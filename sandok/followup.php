...
$limit = isset($_POST["limit-records"]) ? $_POST["limit-records"] : 50;
	...
	$start = ($page - 1) * $limit;
	$result = $conn->query("SELECT * FROM deathinfo  LIMIT $start,$limit");
	$customers = $result->fetch_all(MYSQLI_ASSOC);
 $countA = $conn->query("SELECT COUNT(DISTINCT id) as id FROM deathinfo");
...
if($page < 1){
  ...
  <td>
                ...
                <input type="text" value="<?php echo $customer['ID'];?>" name="edit_id" hidden><button class="btn btn-sm  backButton" type="submit" style="background-color:green; color:white;" name="confirm">طلب صرف</button>
                </form></td>
