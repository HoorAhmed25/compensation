...
$limit = isset($_POST["limit-records"]) ? $_POST["limit-records"] : 50;
	...
	$start = ($page - 1) * $limit;
	$result = $conn->query("SELECT * FROM deathinfo where type = 'وفاة' and governorates = '$governorate' OR workingname = '$workingname' LIMIT $start,$limit");
	$customers = $result->fetch_all(MYSQLI_ASSOC);
 $countA = $conn->query("SELECT COUNT(DISTINCT id) as id FROM deathinfo where type = 'وفاة'");
...
if($page < 1){
  ...
  <div class="container mt-5" style="border: 1px solid #eeeeee;">
	...
	$page = isset($_GET['page']) ? $_GET['page'] : 1;
	$start = ($page - 1) * $limit;
	$result = $conn->query("SELECT * FROM deathinfo where type = 'وفاة' LIMIT $start,$limit");
	$customers = $result->fetch_all(MYSQLI_ASSOC);
 $countA = $conn->query("SELECT COUNT(DISTINCT id) as id FROM deathinfo where type = 'وفاة'");
         $custCountA = $countA->fetch_all(MYSQLI_ASSOC);
	$totalCustomer = $custCountA[0]['id'];
