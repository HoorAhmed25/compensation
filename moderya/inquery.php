...
if(isset($_POST['search'])){
    ...
    $governorate = $result1['gov']; 
$limit = isset($_POST["limit-records"]) ? $_POST["limit-records"] : 50;
	$page = isset($_GET['page']) ? $_GET['page'] : 1;
	$start = ($page - 1) * $limit;
$result = $conn->query("SELECT * FROM deathinfo where type = '$type' OR nationalId = '$nationalID' AND status = 'مديرية' AND 
governorate = '$governorate' LIMIT $start,$limit");
	$customers = $result->fetch_all(MYSQLI_ASSOC);
 $countA = $conn->query("SELECT COUNT(DISTINCT id) as id FROM deathinfo where type = '$type' OR nationalId = '$nationalID' AND status = 'مديرية' AND governorate = '$governorate'");
...
if($page < 1){
  ...
  <div class="container mt-5" style="border: 1px solid #eeeeee;">
    ...
    $ins="SELECT * FROM user WHERE username = '$username'";
...
$limit = isset($_POST["limit-records"]) ? $_POST["limit-records"] : 50;
	$page = isset($_GET['page']) ? $_GET['page'] : 1;
	$start = ($page - 1) * $limit;
	$result = $conn->query("SELECT * FROM deathinfo where status = 'مديرية' AND governorate = '$governorate' LIMIT $start,$limit");
	$customers = $result->fetch_all(MYSQLI_ASSOC);
 $countA = $conn->query("SELECT COUNT(DISTINCT id) as id FROM deathinfo where status = 'مديرية' AND governorate = '$governorate'");
         $custCountA = $countA->fetch_all(MYSQLI_ASSOC);
	$totalCustomer = $custCountA[0]['id'];
	$pages = ceil( $totalCustomer / $limit  );
if($page < 1){
