    ...
    <div class="modal-dialog">
      ...
      <div class="modal-content text-right">
        ...
        <div class="modal-body">
          <textarea class="form-control" cols="20" rows="5" name="reason"></textarea>
        </div>
...
if(isset($_POST['accept'])){
    ...
    $type = $_POST['type'];
    if($type=='وفاة'){
       $ins = "UPDATE deathinfo SET status = 'تأمين صحى' WHERE nationalID = '$ID';";
$query= mysqli_query($conn,$ins) or die("error:".mysqli_error($conn));
        }
    elseif($type == 'مصاب بالعجز'){
         $ins = "UPDATE deathinfo SET status = 'المجالس الطبية' WHERE nationalID = '$ID';";
        $query= mysqli_query($conn,$ins) or die("error:".mysqli_error($conn));
    }
if($query) 
{ 
 ...
 echo '<script type="text/javascript">';echo'window.location.href="inquery.php";';echo '</script>';
         ...
         $ID = $_POST['nationalId'];
        $reason = $_POST['reason'];
       $ins = "UPDATE deathinfo SET status ='مرفوض عودة للمستشفى',reason  = '$reason' WHERE nationalID = '$ID';";
$query= mysqli_query($conn,$ins) or die("error:".mysqli_error($conn));
if($query) 
{ 
 echo '<script type="text/javascript">';
